...
Для минимального старта необходимо установить библиотеки:
...
5.3.2. Для завершения хэширования названий, нам необходимо пробросить в index.html наши 
...
6. Мы можем избавиться в наших компонентах написанных на классах, от конструктора 
(частично, state останится) и необходимости выполнять bind (пробрасывать this в 
другие функции). Это облегчит написание кода и повысит его читаемость. Мы можем 
подключить плагин @babel/plugin-proposal-class-properties  , а затем преобразовать 
state и все функции внутри классов трансформировать в стрелочные функции. Так как 
стрелочная функция не имеет своего this, а наследует this от места где она была 
объявлена, данная библиотека поможет babel правильно развернуть наши функции.

	Полный примет компонента Form

	Установка:
	>npm install --save-dev @babel/plugin-proposal-class-properties

6.1. Production сборка. Фактически это сборка с минифицироваными файлами. Мы можем 
объявить в нашем package.json в скриптах  NODE_ENV=production, затем получить в нашем 
webpack.config.js параметры  NODE_ENV через process.env.NODE_ENV и указать наш 
модификатор сборки mode: env.

6.2. webpack.DefinePlugin.  Мы можем передавать параметры в наш проект из 
webpack.config.js. Для этого ми не устанавливая каких либо новых плагинов, подключаем 
встроенный плагин „new  webpack.DefinePlugin“. Таким образом мы можем передавать 
не публичные параметры.
	Параметры передаем через  JSON.stringify для адекватного их получения. В данном 
случае получения 'dfdfsdfs' в виде строки.

new webpack.DefinePlugin({
__DEV__: isDev,
__NEXT_PARAMS__: JSON.stringify(isDev?'dfdfsdfs':'gwe')
}),

	Далее в проекте подключаем webpack как простой компонент и используем эти переменные. 
Обращение __DEV__ или __NEXT_PARAMS__.

	Это полезно нам, как минимум, на стадии разработки, например при выведении данных 
в консоль. Пример:

	if(__DEV__) console.log(props); //  webpack проигнорирует эту строку при production
	 				        		// сборке, это касается любого кода

6.3. Dotenv - это модуль с нулевой зависимостью, который загружает переменные среды 
из файла .env в process.env. Хранение конфигурации в среде, отдельной от кода.  
Для этого устанавливаем пакет dotenv-webpack

	>npm install -D dotenv-webpack

	В webpack.config делаем импорт и подключаем наш  плагин. 
	Создаем файла .env
	В проекте получаем параметры через process.env.__API_KEY__
